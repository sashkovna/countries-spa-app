        ...
        <Wrapper>
            ...
            <div>
                ...
                <ListGroup>
                    ...
                    <List>
                        <ListItem><b>Top Level Domain: </b>{topLevelDomain.map(d => <span key={d}>{d}</span>)}
                        </ListItem>
                        <ListItem><b>Currencies: </b>{currencies.map(c => <span key={c.code}>{c.name} </span>)}
                        </ListItem>
                        <ListItem><b>Languages: </b>{languages.map(l => <span key={l.name}>{l.name} </span>)}</ListItem>
                    </List>
                </ListGroup>
                <Meta><b>Border countries</b>
                    {!borders.length
                        ? <span>There is no border countries</span>
                        : <TagGroup>
                            {neighbors.map(n => <Tag key={n} onClick={() => navigate(`/country/${n}`)}>{n} </Tag>)}
                        </TagGroup>
                    }
                </Meta>
            </div>
